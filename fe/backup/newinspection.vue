        ...
        <template #text>
          ...
          <div v-if="dialogData.key == 'new'">
            <v-row>
              ...
              <v-col cols="12" v-if="partSelected">
    ...
    Object.entries(e).forEach(([key, value]) => {
      if (parseObject.includes(key)) {
        if (typeof value === "string") {
          ...
          part.value[key] = value; // sudah object, langsung assign
        }
      }

      if (boolObject.includes(key)) {
        part.value[key] = value === 1 || value === "1" || value === true;
      }
    });

    partSelected.value = true;
  }
});

watch(dialog, (e) => {
  if (!e) {
    partSelected.value = e;
    setTimeout(() => {
      part.value = null;
    }, 500);
  }
...
function extrudingAdd(item) {
  ...
  exportData.inspectionData.extruding = {
    pic: user,
...
function kneadingAdd(item) {
  ...
  exportData.step = `${item.pos}|0`;
  exportData.inspectionData.kneading = {
    pic: user,
    time: moment().format("YYYY-MM-DD"),
  };
  nextTick().then(() => {
...
function pressAdd(item) {
  ...
  exportData.step = `${item.pos}|0`;
  exportData.inspectionData.press = {
    pic: user,
    time: moment().format("YYYY-MM-DD"),
  };
...
function outgoingAdd(item) {
  let exportData = { ...toRaw(selected.value) };
  exportData.judgement = item.ng ? "NG" : null;
  exportData.outgoingData = item.tableData;
  exportData.step = `${item.pos}|0`;
  exportData.inspectionData.outgoing = {
    pic: user,
    time: moment().format("YYYY-MM-DD"),
  };
...
function kneadingAbuffer() {
  ...
  localSelected.inspectionData.kneading = {
    pic: user,
    time: moment().format("YYYY-MM-DD"),
  };
  nextTick().then(() => {
...
async function refresh() {
  ...
  inspection.value = await store.ajax({}, "/inspection", "post");
  store.preload = false;
}
async function validateAppFinal() {
  const appJudgement = validateInputs(selected.value.apperanceData);

  if (appJudgement) {
    extStep.value++;
  } else {
    let item = { ...toRaw(selected.value) };

    item.ngData.push("appearance");
    item.judgement = "NG";
    Object.entries(item).forEach(([key, value]) => {
      if (value == null) {
        delete item[key];
    ...
    await store.ajax(item, "/inspection/appearanceng", "post");
    refresh();
  }
}
